...
class TrinityDynamicNTKScalingRoPE: Module {
    let dims: Int
...
class TrinityAttention: Module {
    ...
    let rope: TrinityDynamicNTKScalingRoPE

    init(_ args: TrinityConfiguration) {
        self.args = args
        ...
        self.rope = TrinityDynamicNTKScalingRoPE(
            dims: headDim,
...
class TrinityMLP: Module, UnaryLayer {
...
class TrinityTransformerBlock: Module {
    @ModuleInfo(key: "self_attn") var attention: TrinityAttention
    @ModuleInfo(key: "mlp") var mlp: TrinityMLP

    @ModuleInfo(key: "input_layernorm") var inputLayerNorm: RMSNorm
    @ModuleInfo(key: "post_attention_layernorm") var postAttentionLayerNorm: RMSNorm

    init(_ args: TrinityConfiguration) {
        self._attention.wrappedValue = TrinityAttention(args)
        self._mlp.wrappedValue = TrinityMLP(args)
...
public class TrinityModelInner: Module {

    @ModuleInfo(key: "embed_tokens") var embedTokens: Embedding

    let layers: [TrinityTransformerBlock]
    let norm: RMSNorm
    ...
    init(_ args: TrinityConfiguration) {
        ...
        self.layers = (0 ..< args.hiddenLayers).map { _ in TrinityTransformerBlock(args) }
...
public class TrinityModel: Module, LLMModel, KVCacheDimensionProvider {

    public let vocabularySize: Int
    public let kvHeads: [Int]

    public let model: TrinityModelInner

    @ModuleInfo(key: "lm_head") var lmHead: Linear?

    public init(_ args: TrinityConfiguration) {
        self.vocabularySize = args.vocabularySize
        self.kvHeads = (0 ..< args.hiddenLayers).map { _ in args.kvHeads }
        self.model = TrinityModelInner(args)
...
public struct TrinityConfiguration: Codable, Sendable {
...
extension TrinityModel: LoRAModel {
